...
# Konversi kolom order_approved_at menjadi tipe data datetime
...
with st.container():
    ...
    month_df = all_df.resample(rule='M').agg({
        "order_id": "nunique",
    })
...
# Membuat tiga grafik untuk setiap tab
...
sns.barplot(x="monetary", y="customer_id", data=rfm_df.sort_values(by="monetary", ascending=False).head(5), palette=colors, ax=ax3)
ax3.set_ylabel(None)
ax3.set_xlabel(None)
